...
### 2026-07-27 · Status artwork pass 2

- 去除状态框 PNG 外围与旧三段式 SVG 降级层叠加造成的暗色矩形；生成图加载成功后只显示完整绘制框，加载失败时仍保留 SVG 框架。
- 为心跳、雨景、安静三项状态分别制作透明水晶球 PNG，替换原先“CSS 圆球 + 线性 SVG 图标”的扁平组合。
- 三枚图标统一裁切为透明 384 × 384 高分屏素材；状态文案和实时值继续由 DOM 渲染，不写入图片。
- PWA shell 缓存升级为 `luminous-shell-v5`，视觉验收增加三枚状态图标的加载断言。

### 2026-07-27 · Portal artwork pass 1

- 四个子页面入口切换为独立透明 PNG：今日/月相、记忆/晶体、来信/信封、心迹/莲花。
- 入口按钮、`data-space` 路由、无障碍标签和移动端布局保持原样；美术图加载失败时自动回退到原 SVG 图标。
- 素材来自 `ui.png` 的真实入口绘制层提取，并在本地做圆形羽化与 384px 高分屏输出；待图像生成服务恢复后可替换为独立生成版本。

### 2026-07-27 · Status and composer proportion pass

- 状态组件加高并同步放大三枚状态球，使绘制框的长宽比和信息密度更接近 `ui.png`。
- 输入框装饰层不再向容器外扩，输入框左缘、主体宽度和底部位置按参考图重新对齐。
- 桌面发送水晶球放大到 136px，并扩大内部纸飞机素材；移动端保持独立 62px 尺寸，避免挤压文本输入区域。
- PWA shell 缓存升级为 `luminous-shell-v7`。
...
| 3 | `ui.png`、状态栏第二轮 | 接入三枚独立水晶状态球，修正绘制框与降级框叠层 | 178 项测试、Playwright 资源加载断言、桌面/移动截图 | 已完成 |
| 4 | `ui.png`、四个入口图标 | 四个复杂入口图标资产化，保留 SVG 降级 | Playwright 资源加载断言、桌面/移动截图 | 已完成 |
| 5 | `ui.png`、状态与输入区比例 | 调整状态框、输入框和发送按钮几何比例 | Playwright 桌面/移动截图 | 进行中 |
